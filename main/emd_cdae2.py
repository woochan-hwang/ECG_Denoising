...
CAE4 = ConvAutoEncoder()
CAE5 = ConvAutoEncoder()
loss_func = nn.L1Loss()
...
optimizer4 = torch.optim.Adam(CAE4.parameters(), lr=LR3, weight_decay=1e-5)
optimizer5 = torch.optim.Adam(CAE5.parameters(), lr=LR3, weight_decay=1e-5)
...
def save_model(save_name, optim, loss_f, lr, epoch = EPOCH):
    ...
    torch.save({'data_setting': data,
                ...
                'state_dict_4': CAE4.state_dict(),
                'state_dict_5': CAE5.state_dict(),
                'epoch': epoch,
                'optimizer': optim,
...
try:
    ...
    for epoch in range(EPOCH):
        for step, train_data in enumerate(train_loader):
            ...
            b_x4 = Variable(x_IMFs[:,3:4,:,:]).cuda() if cuda else Variable(x_IMFs[:,3:4,:,:])
            b_y4 = Variable(y_IMFs[:,3:4,:,:]).cuda() if cuda else Variable(y_IMFs[:,3:4,:,:])

            b_x5 = Variable(x_IMFs[:,4:5,:,:]).cuda() if cuda else Variable(x_IMFs[:,4:5,:,:])
            b_y5 = Variable(y_IMFs[:,4:5,:,:]).cuda() if cuda else Variable(y_IMFs[:,4:5,:,:])

            de1, de2, de3, de4, de5 = CAE1(b_x1), CAE2(b_x2), CAE3(b_x3), CAE4(b_x4), CAE5(b_x5)
            ...
            loss4 = loss_func(de4, b_y4)
            optimizer4.zero_grad()
            loss4.backward()
            optimizer4.step()

            loss5 = loss_func(de5, b_y5)
            optimizer5.zero_grad()
            loss5.backward()
            optimizer5.step()

        print('Epoch loss: {} | IMF 1: {:.4f} | IMF 2: {:.4f} | IMF 3: {:.4f} | IMF 4: {:.4f} | IMF 5: {:.4f}'.format(epoch+1, loss1.data[0], loss2.data[0], loss3.data[0], loss4.data[0], loss5.data[0]))
        train_loss.append((loss1.data[0], loss2.data[0], loss3.data[0], loss4.data[0], loss5.data[0]))
        ...
        if epoch % 10 == 0:
            ...
            val_loss.append((loss1.data[0], loss2.data[0], loss3.data[0], loss4.data[0], loss5.data[0]))
...
#            val_loss.append((train_recon_loss,val_recon_loss))
    ...
    save_model('EMD_CDAE_v3_nl{}'.format(noiselevel), 'Adam', 'L1Loss', LR1)
